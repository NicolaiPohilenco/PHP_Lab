...
<body>
    <?php if (!isset($_REQUEST['start'])) { ?>
        ...
        <form action="<?php echo $_SERVER['SCRIPT_NAME'] ?>" method="post">
            ...
            <div>
                <label>Ваш отзыв:
                    <textarea name="message" cols="40" rows="4" placeholder="Отзыв..."></textarea>
                </label>
            </div>
            <div>
                <input type="reset" value="Очистить" />
                <input type="submit" value="Отправить" name="start" />
            </div>
        </form>
    </body>
...
<?php } else {
        ...
        $file = fopen('../lab5-6_PohilencoNicolai/messages.txt', 'a+') or die("Недоступный файл!");
        ...
        $file = fopen("../lab5-6_PohilencoNicolai/messages.txt", "r") or die("Unavailable file!");
        while (!feof($file)) {
            echo fgets($file) . "<br />";
        }
        fclose($file);
